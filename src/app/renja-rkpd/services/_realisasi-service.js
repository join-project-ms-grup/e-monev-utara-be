...
const mappingIndikator = (indikator) => {
       ...
       for (const ind of indikator) {
              ...
              const rincian = ind.rincian[0];
              const capaianList = rincian.capaian || [];
              const jenis = ind.jenis_satuan; // "akumulatif" | "negatif" | "tetap"

              if (capaianList.length === 0) {

                     // Jika belum ada data capaian -> isi default
                     capaian = {
                            id_rincian: rincian.id,
                            tahun_ke: rincian.tahun_ke,
                            target: rincian.target,
                            capaianTriwulan: [1, 2, 3, 4].map(tri => ({
                                   triwulan: tri,
                                   capaian: 0
                            })),
                            capaianTotal: 0,
                     ...
                     // Bentuk array capaianTr
                     const capaianTriwulan = capaianList.map(item => ({
                            triwulan: item.triwulan,
                            capaian: item.capaian
                     }));

                     // --- Hitung capaianTotal sesuai jenis ---
                     let capaianTotal;

                     if (jenis === "akumulatif") {
                            // jumlah seluruh capaian
                            capaianTotal = capaianList.reduce((sum, item) => sum + item.capaian, 0);

                     } else if (jenis === "negatif") {
                            // nilai terendah
                            capaianTotal = Math.min(...capaianList.map(x => x.capaian));

                     } else if (jenis === "tetap") {
                            // ambil capaian triwulan terakhir yang terisi
                            const last = capaianList.sort((a, b) => b.triwulan - a.triwulan)[0];
                            capaianTotal = last.capaian;

                     } else {
                            // fallback jika data jenis tidak valid
                            capaianTotal = 0;
                     }

                     // Hitung persen
                     const persenCapaian = rincian.target > 0
                            ? ((capaianTotal / rincian.target) * 100).toFixed(2)
                            : "0.00";
