...
export const sinkronisasiSKPD = async (req) => {
       ...
       for (const skpd_id of skpdCollectIds) {
              const exits = await prisma.skpd_periode.findFirst({ where: { skpd_id, periode_id } });
              if (exits) {
                     ...
                     const setSKPD_periode = await prisma.skpd_periode.create({
...
export const statusToggle = async (req) => {
       ...
       const exist = await prisma.skpd_periode.findUnique({
              where: { id },
       });
       ...
       await prisma.skpd_periode.update({
              where: { id },
